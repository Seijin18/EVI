...
def reply_prefix() -> str:
    """Always returns '[EVI] ' (label + single trailing space)."""
...
def format_evi_whatsapp(text: str) -> str:
    ...
    if body.startswith(prefix) or body.startswith(prefix.strip()):
        # Normalize missing space after [EVI]
...
def send_whatsapp_text(jid: str, text: str, *, add_prefix: bool = True) -> bool:
    """Send text to a WhatsApp JID via Evolution sendText."""
    ...
    payload = {
        "number": number,
        "text": format_evi_whatsapp(text) if add_prefix else text[:4000],
    }
